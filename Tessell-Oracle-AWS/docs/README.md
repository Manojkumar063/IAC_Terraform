# 🗄️ Tessell Oracle DB on AWS — Terraform Provisioning Guide

> Provision an **Oracle 19c** database on **AWS** using **Tessell DBaaS** and **Terraform** — step by step, from zero to a running database.
...
```
You write .tf files  →  terraform apply  →  Tessell API  →  Oracle DB running on AWS
```
...
| **VPC Name** | Tessell Console → Infrastructure → Networks |
...
resource "tessell_db_service" "oracle_dev_qa" {
  ...
  # Basic Info
  name        = "oracle-dev-qa"
  description = "Oracle 19c Dev/QA DB on AWS"
...
```bash
mkdir tessell-oracle-aws && cd tessell-oracle-aws
...
Type `yes` to confirm. This will delete the Oracle DB service from Tessell/AWS.
...
- ✅ For team use: store `terraform.tfstate` in a remote backend (S3 or HCP Terraform)
...
*Generated for: Oracle 19c SE2 | AWS us-east-1 | Dev/QA | single_instance topology*
